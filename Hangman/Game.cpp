...
namespace Hangman {
    ...
    void Game::Run()
    {
        ...
        while (!word.IsFullyRevealed())
        {
            ...
            figure.Display();
            word.Display();
            const char guess = player.GetUserInput(std::cin);
    ...
    std::vector<std::string> Game::getLines(std::ifstream& file) const {
    ...
    std::string Game::GetWord(std::string filename) const {
